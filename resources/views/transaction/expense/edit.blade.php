    ...
    <div class="container-fluid">
        ...
        <div class="col-lg-12">
            <div class="card-style mb-30">
                ...
                <form id="confirm-form" action="{{ route('expense.update', ['expense' => $expense->id]) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-6">
                        <div class="input-style-1">
                            ...
                            <input type="date" id="date" name="date" placeholder="Masukan tanggal" value="{{ old('date', $expense->date) }}" />
                            @error('date')
                        ...
                        <div class="input-style-1">
                          ...
                          <input type="text" id="name" name="name" placeholder="Masukan nama pemasukan" value="{{ old('name', $expense->name) }}" />
                          @error('name')
                              <div class="text-danger text-sm mt-2">{{ $message }}</div>
                          @enderror
                        </div>
                            ...
                            <div class="select-position">
                                <select name="wallet_id" id="wallet_id" required>
                                    ...
                                    @foreach ($wallets as $wallet)
                                        <option value="{{ $wallet->id }}" data-currency="{{ $wallet->currency }}" {{ old('wallet_id', $expense->wallet_id) == $wallet->id ? 'selected' : '' }}>{{ $wallet->name }} {{ $wallet->account_number ? '- ' . $wallet->account_number : '' }} {!! $wallet->status == 'active' ? '<span class="text-success">(Aktif)</span>' : '<span class="text-danger">(Nonaktif)</span>' !!}</option> 
                                    @endforeach
                                </select>
                            </div>
                            @error('wallet_id')
                    ...
                    <div class="col-6">
                        <div class="input-style-1">
                          ...
                          <input type="number" id="amount" name="amount" placeholder="Masukan jumlah" value="{{ old('amount', $expense->amount) }}" />
                          @error('amount')
                              <div class="text-danger text-sm mt-2">{{ $message }}</div>
                          @enderror
                        </div>
                        <div class="input-style-1" id="exchangeRateWrapper" style="display: none;">
                            ...
                            <input type="number" id="exchange_rate" name="exchange_rate" placeholder="Masukan jumlah" value="{{ old('exchange_rate', $expense->exchange_rate) }}" />
                            @error('exchange_rate')
                                <div class="text-danger text-sm mt-2">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="input-style-1">
                            ...
                            <textarea placeholder="Masukan deskripsi" id="description" name="description" rows="3">{{ old('description', $expense->description) }}</textarea>
                            @error('description')
                                <div class="text-danger text-sm mt-2">{{ $message }}</div>
                            @enderror
                        </div>
